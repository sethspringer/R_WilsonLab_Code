...
for (X_current in 1: dimensions[1]) 
{
  for (Y_current in 1: dimensions[2])
  {
    for (Z_current in 1: dimensions[3])
    {
      ...
      y = full_4D_matrix_c1[X_current,Y_current,Z_current,]
      ...
      if (sum(y) == 0) { 
        ...
        #xx switch this back when done testing
          ...
          results = mediate(model.a, model.Whole, treat='x', mediator='m', boot=TRUE, sims = 5000)
...
print("Sorry but no mediation bootstrapping was ran since no voxels met the mediation requirements")
  ...
  img.nifti@bitpix = 64
  fname = file.path(path = save_path, 'WholeBrainMediation_pValue')
  ...
  img.nifti@datatype = 64
  img.nifti@bitpix = 64
  fname = file.path(path = save_path, 'WholeBrainMediation_effectCoeff')
  writeNIfTI(nim =img.nifti, fname, gzipped = FALSE) #gzipped = false to prevent the .nz in the file  
  ...
  img.nifti@datatype = 64
  img.nifti@bitpix = 64
  fname = file.path(path = save_path, 'WholeBrainMediation_scaledSignificance_largerValueMeansMoreSig')
  writeNIfTI(nim =img.nifti, fname, gzipped = FALSE) #gzipped = false to prevent the .nz in the file
