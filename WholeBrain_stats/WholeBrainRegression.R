...
r_stat_map <- array(c(0), dim = c(dimensions[1], dimensions[2], dimensions[3]))
...
for (X_current in 1: dimensions[1]) 
{
  for (Y_current in 1: dimensions[2])
  {
    for (Z_current in 1: dimensions[3])
    {
      ...
      model.brain_model = lm(pseudo_Ts_temp ~ data_predictor) #the number indicates the column!
      
      corr_r = cor(pseudo_Ts_temp,data_predictor)
      ...
      r_stat_map[X_current, Y_current, Z_current] = corr_r
...
t_stat_output_name = paste('lm_regression_t_values_', '_df_', df_2,sep = '')
...
F_stat_output_name = paste('lm_regression_F_values_', '_df_', df_1, '_',df_2,sep = '')
...
r_stat_output_name = paste('lm_regression_r_values_', '_df_', df_2,sep = '')


#Write out the t stats brain
pixdim = pixdim(img)
img.nifti = as.nifti(from = r_stat_map, value = hdr,  verbose = FALSE) 
img.nifti@reoriented = T
fname = file.path(path = save_path, r_stat_output_name)
writeNIfTI(nim =img.nifti, fname, gzipped = FALSE) #gzipped = false to prevent the .nz in the file
